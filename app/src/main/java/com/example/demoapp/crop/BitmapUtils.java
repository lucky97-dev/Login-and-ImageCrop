...
final class BitmapUtils {
  ...
  static BitmapSampled decodeSampledBitmap(Context context, Uri uri, int reqWidth, int reqHeight) {
    ...
    try {
      ...
      options.inSampleSize =
          Math.max(calculateInSampleSizeByReqestedSize(options.outWidth, options.outHeight, reqWidth, reqHeight),
  ...
  private static Bitmap cropBitmapObjectWithScale(
      ...
      boolean flipVertically) {
    Rect rect =
        getRectFromPoints(points, bitmap.getWidth(), bitmap.getHeight(), fixAspectRatio, aspectRatioX, aspectRatioY);
    ...
    try {
      ...
      if (fullBitmap != null) {
        try {
          ...
          result =
              cropBitmapObjectWithScale(fullBitmap, points2, degreesRotated, fixAspectRatio, aspectRatioX, aspectRatioY, 1, flipHorizontally, flipVertically);
        } finally {
